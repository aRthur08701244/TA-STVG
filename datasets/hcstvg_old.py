...
class HCSTVGDataset(data.Dataset):
    ...
    def load_frames(self, data_item, load_video=True):
        ...
        patience = 20
        resolution = self.cfg.INPUT.RESOLUTION
        max_rate = 1.4
        ...
        if load_video:
            ...
            frames = [Resize((resolution, min(int(resolution*rate), int(resolution*max_rate))), antialias=True)(ToTensor()(frame)) for frame in frames]
